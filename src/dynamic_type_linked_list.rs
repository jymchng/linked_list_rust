...
impl Node {
  ...
  pub fn borrow_next(&self) -> Option<Ref<Node>> {
    self.next.as_ref().map_or(None, |next_| Some(next_.borrow()))
  }

  pub fn borrow_next_mut(&self) -> Option<RefMut<Node>> {
    self.next.as_ref().map_or(None, |next_| Some(next_.borrow_mut()))
  }
...
impl DynLinkedList {
  ...
  pub fn borrow_head(&self) -> Option<Ref<Node>> {
    self.head.as_ref().map_or(None, |next_| Some(next_.borrow()))
  }

  pub fn borrow_tail(&self) -> Option<Ref<Node>> {
    self.tail.as_ref().map_or(None, |next_| Some(next_.borrow()))
  }
  ...
  pub fn append(&mut self, node: Node) {
    ...
    self.tail.take().map_or_else(
      ...
      |old_tail| {
        old_tail.borrow_mut()
                .set_wrapped_next(wrapped_node.clone())
      });
